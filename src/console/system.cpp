...
std::string System::dataPath(const std::string& file) const {
    ...
    if (base.empty()) {
        char* p = SDL_GetPrefPath("macncrash", "sunsetcruise");
        base = p ? p : "./";
        SDL_free(p);
    }
    return base + file;
...
#ifdef __EMSCRIPTEN__
    std::string js = "localStorage.getItem('cruise-" + name + "') || ''";
...
void System::saveBlob(const std::string& name, const std::string& data) const {
    if (headless) return;
#ifdef __EMSCRIPTEN__
    EM_ASM({ try { localStorage.setItem('cruise-' + UTF8ToString($0), UTF8ToString($1)); } catch (e) {} }, name.c_str(), data.c_str());
#else
