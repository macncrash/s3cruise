...
Bitmap logoBitmap() {
    Bitmap b(320, 104);
    Bitmap l1 = gs::textBitmap("SUNSET", {4, 5, 15, 0, 1});
    Bitmap l2 = gs::textBitmap("CRUISE", {6, 1, 15, 0, 1});
...
// The low sun, sitting on the Pacific (far palette 8-9), with its glitter path on the sea (11-12).
void sunOnSea(Bitmap& b, Rng& r, float cx, float cy, float rad) {
    for (int o : {0, b.w}) {
        b.ellipse(cx - o, cy, rad, rad, 9);
        b.ellipse(cx - o, cy, rad * 0.75f, rad * 0.75f, 8);
    }
    b.rect(0, 232, float(b.w), 24, 11);
    for (int y = 234; y < 256; y += 2)
        for (int k = 0; k < 3; k++) {
            const float w = 6 + (y - 232) * 2.5f + r() * 8;
            b.rect(cx - w / 2 + (r() - 0.5f) * 20, float(y), w, 1, 12);
        }
}

void backdropFar(Bitmap& b, int stage, Rng& r) {
    switch (stage) {
        case 0:  // Santa Monica: the sun over the bay, Malibu's hills along the shore
            clouds(b, r, 5, 40, 150);
            mountains(b, r, 232, 16, 0, false);
            sunOnSea(b, r, 330, 214, 34);
            break;
        case 1:  // Big Sur: the coast range falling into the sea
            clouds(b, r, 4, 30, 120);
            mountains(b, r, 224, 44, 0, true);
            b.rect(0, 238, float(b.w), 18, 11);
            break;
        case 2:  // Monterey: sea fog rolling in
            mountains(b, r, 236, 10, 0, false);
            clouds(b, r, 14, 150, 235);
            break;
        default: {  // the Golden Gate: towers against the sunset, the city beyond
            sunOnSea(b, r, 360, 206, 40);
            clouds(b, r, 5, 40, 150);
            for (float x = 20; x < 240; x += 7 + r() * 9) {  // skyline
                const float h = 12 + r() * 40;
                b.rect(x, 232 - h, 6 + r() * 6, h, r() < 0.5f ? 3 : 4);
                for (int k = 0; k < 4; k++) b.set(int(x + 2 + r() * 4), int(232 - h + 3 + r() * (h - 4)), 8);
            }
            for (float tx : {300.0f, 420.0f}) {  // bridge towers and the cable
                b.rect(tx, 150, 8, 82, 5);
                b.rect(tx - 4, 170, 16, 4, 5);
                b.rect(tx - 4, 200, 16, 4, 5);
            }
            for (int x = 240; x <= 480; x++) {
                const float t = (x - 360) / 60.0f;
                const float y = x < 300 || x > 420 ? 150 + (std::fabs(x - 360.0f) - 60) * 1.2f : 152 + 40 * (1 - t * t);
                b.set(x, int(std::min(231.0f, y)), 5);
            }
            b.rect(240, 226, 240, 3, 5);
            break;
        }
    }
}
...
void backdropNear(Bitmap& b, int stage, Rng& r) {
    switch (stage) {
        case 0:  // beach: dunes and palms
            hills(b, r, 242, 5, 8, 9);
            for (int i = 0; i < 26; i++) {
                const float x = r() * b.w, h = 14 + r() * 16;
                for (int o : {-b.w, 0, b.w}) {
                    b.line(x + o, 244, x + o + 3, 244 - h, 12, 2);
                    for (int f = 0; f < 5; f++) b.line(x + o + 3, 244 - h, x + o + 3 + (f - 2) * 5, 244 - h + 3 + (f % 2) * 2, 3, 2);
                }
            }
            break;
        case 1:  // Big Sur: rock and scrub
            hills(b, r, 238, 10, 10, 1);
            treeline(b, r, 246, 6, 18, 3, 4, 2);
            break;
        case 2:  // Monterey cypress, wind-bent
            treeline(b, r, 244, 8, 22, 3, 4, 3);
            break;
        default:  // the bay, gold under the sun
            treeline(b, r, 234, 4, 10, 3, 12, 3);
            b.rect(0, 238, float(b.w), float(b.h) - 238, 6);
            for (int y = 239; y < b.h; y += 2)
                for (int k = 0; k < 3; k++) {
                    const float w = 8 + (y - 238) * 3 + r() * 10;
                    const float cx = 360 + (r() - 0.5f) * 30;
...
void buildArt(gs::VDP& vdp, Art& a) {
    ...
    a.obj[O_ARCH_START] = gs::uploadMipped(vdp, arch("SUNSET CRUISE"));
